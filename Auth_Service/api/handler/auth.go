...
func (h *Handler) Register(c *gin.Context) {
	...
	if err != nil {
		h.Log.Error("Error hashing password", err)
...
// @Tags auth
// @Param data body models.RefreshToken true "Access token"
// @Success 200 {string} string "Access token is valid"
// @Failure 400 {object} string "Invalid data"
// @Failure 500 {object} string "Server error while processing request"
// @Router /validate [post]
func (h *Handler) ValidateToken(c *gin.Context) {
	h.Log.Info("ValidateToken handler is invoked")
	...
	valid, err := tokens.ValidateRefreshToken(req.Token)
	if !valid || err != nil {
		er := errors.Wrap(err, "invalid data").Error()
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": er})
		h.Log.Error(er)
	...
	h.Log.Info("ValidateToken handler is completed successfully")
	c.JSON(http.StatusOK, "Refresh token is valid")
}
